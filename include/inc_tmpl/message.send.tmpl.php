		...
		if($result = mysql_query($sql, $db)) {
			if($msgdetail = mysql_fetch_array($result)) {
				...
				$msg_message = 	"\n\n----[".$BL['be_msg_by']." ".$msgdetail["usr_name"].
								" ".$BL['be_msg_on']." ".$msgdetail["send_date"]."]----\n".
								$BL['be_msg_subject'].": ".$msgdetail["msg_subject"].
								"\n".$BL['be_msg_msg'].": ".$msgdetail["msg_text"];
				$msg_to = $msgdetail["msg_from"];
				$msg_pid = $msg;
			}
		}
	}
...
if($result = mysql_query(	"SELECT detail_signature FROM ".DB_PREPEND."phpwcms_userdetail WHERE detail_pid=".
							$_SESSION["wcs_user_id"]." LIMIT 1;") ) {
	if($row = mysql_fetch_row($result)) {
		if(trim($row[0])) $msg_message = "\n\n\t\n".$row[0].$msg_message;
	}
}
